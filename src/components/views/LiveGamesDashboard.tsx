...
function LiveGameCard({ game }: { game: ScheduleGame }) {
  ...
  return (
    <button
      ...
      className="group min-h-60 touch-manipulation rounded-2xl border border-gray-800 bg-gray-900 p-5 text-left shadow-[0_18px_50px_rgba(0,0,0,0.28)] transition-colors hover:border-gray-600 hover:bg-gray-900/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
    >
...
export function LiveGamesDashboard({ schedule }: { schedule: SeasonSchedule | null }) {
  ...
  return (
    ...
    >
      <div hidden={!!selectedGame} data-testid="live-games-overview">
      ...
      {selectedGame && (
        <div ref={detailsRef} className="scroll-mt-6" data-testid="selected-game-details">
          <GameStatsDialog
            open
            variant="panel"
            onOpenChange={(open) => {
