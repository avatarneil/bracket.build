...
export function LiveFieldPosition({
  ...
  return (
    <section
      ...
      className="border-b border-gray-700 px-4 py-4 md:px-6"
    >
      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex items-center gap-4 text-[11px] font-medium text-gray-300">
          <span className="inline-flex items-center gap-1.5">
            <span className="h-3 w-0.5 bg-white" aria-hidden="true" /> Line of scrimmage
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="h-3 w-0.5 bg-yellow-300" aria-hidden="true" /> First down
          </span>
        </div>
      </div>
      ...
      <div
        ...
        aria-label={
          fieldPosition
            ...
            : "The game is between drives. Field position will appear when the next line of scrimmage is established."
        }
        className="relative h-32 overflow-hidden rounded-xl border-2 border-white/70 bg-[#176b3a] shadow-[inset_0_0_30px_rgba(0,0,0,0.28)]"
      >
      ...
      {lastPlay && <p className="mt-3 line-clamp-2 text-xs leading-5 text-gray-400">{lastPlay}</p>}
    </section>
  );
}
