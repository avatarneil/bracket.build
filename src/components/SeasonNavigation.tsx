...
export function SeasonNavigation({
  ...
  return (
    <nav
      aria-label={league === "nfl" ? "NFL season" : "College football season"}
      className="@container min-w-52 flex-1"
    >
      <div
        className={cn(
          "grid rounded-xl bg-gray-900 p-1",
          league === "nfl" ? "grid-cols-3" : "grid-cols-2",
        )}
        data-testid="season-navigation"
      >
        {PHASES.filter((phase) => league === "nfl" || phase.id !== "preseason").map(
            ...
            return (
              ...
              >
                ...
                <span className="@min-md:hidden">
                  {league === "college-football" && id === "postseason"
                    ? "Bowls / CFP"
                    : shortLabel}
                </span>
